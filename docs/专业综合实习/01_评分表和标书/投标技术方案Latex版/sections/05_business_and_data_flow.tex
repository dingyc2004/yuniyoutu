...
\begin{enumerate}
  \item \textbf{读路径}：前端组件 → \texttt{api.js}（HTTP 请求）→ Vite 代理 \texttt{/api} → FastAPI 路由 → Service 层调用 \texttt{load\_collection()} → 读取 \texttt{data/*.json} → 返回结构化 JSON → 前端渲染
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3cm}|p{4cm}|p{8cm}}
    ...
    \midrule
    FishingPOI（钓点） & \texttt{data/pois.json} & 被 FishingRecord、CatchPost、Recommendation 引用；与高德 POI ID 关联 \\
    \hline
    FishingRecord（出钓记录） & \texttt{data/records.json} & 关联 POI/钓点名称、用户（user\_id）、天气快照；可作为帖子来源 \\
    \hline
    CatchPost（社区帖子） & \texttt{data/posts.json} & 可关联记录（record\_id）、POI（poi\_id）、用户（user\_id） \\
    \hline
    ...
    \hline
    Event / Order / Message（服务演示） & \texttt{data/events.json}、\texttt{data/orders.json}、\texttt{data/messages.json} & 支撑活动报名、签到、群聊消息、装备订单等 Demo 闭环 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{itemize}
  \item 一条 FishingRecord 隶属于一个用户，可关联钓点名称、坐标和天气快照
  \item 一条 CatchPost 可选关联一条 FishingRecord（从记录一键生成帖子），此时帖子可追溯到原始出钓数据
...
当前 MVP 阶段已实现上述主流程中的关键节点：地图首页展示 POI、POI 详情查看、推荐接口返回评分与理由、记录 CRUD、帖子发布与社区流展示、个人统计汇总。评论、点赞、收藏、关注、私信、活动报名/签到、群聊消息、教程进度和订单演示已作为扩展 Demo 接入同一 JSON 数据流。数据持久化通过 JSON 原子写入实现，服务重启后数据不丢失；正式多人使用时可在该数据流基础上迁移到 PostgreSQL/PostGIS、对象存储和统一鉴权。
