...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3cm}|p{6cm}|p{5cm}}
    ...
    \hline
    数据持久层 (\texttt{data/json\_store.py}) & JSON 集合的加载、保存、原子更新 & load\_collection、save\_collection、update\_collection（临时文件 + 原子替换） \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{0.6cm}|p{3.5cm}|p{4.5cm}|p{5.5cm}}
    ...
    \hline
    5 & 数据结构与数据可用性 & 5 & 核心模型（FishingPOI、CatchPost 等）字段完整，JSON 集合配 schema 文件 \\
    \hline
