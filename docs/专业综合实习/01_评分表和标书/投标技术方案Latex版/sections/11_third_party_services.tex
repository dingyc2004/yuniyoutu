...
本章说明平台第三方服务的接入方式、责任边界、密钥管理和降级策略。当前 MVP 阶段重点接入高德 JS API、后端天气/POI 封装能力以及 DeepSeek AI 解释接口；对象存储、CDN、HTTPS 证书自动化和更完整的网关治理属于正式工程化阶段。系统设计要求第三方服务由后端统一封装，前端只获取必要的展示配置和业务结果，避免 Web 服务 Key、AI Key 等敏感信息暴露在前端代码中。
...
\subsection{对象存储与 CDN（后续规划）}

当前课程 MVP 以页面演示和 JSON 数据为主，不把对象存储作为本期已接入能力。正式工程化阶段，鱼获图片、帖子图片、用户头像、教程视频封面和报告长图应存放在对象存储中，并通过 CDN 加速访问；数据库只保存文件 URL、文件类型、大小、归属用户、关联业务 ID 和审核状态，不直接存放大文件。
...
\subsection{HTTPS、域名与证书（后续规划）}

正式上线时，平台应通过 HTTPS 提供服务，避免用户位置、登录凭证、发帖内容和图片上传信息在传输过程中被窃听或篡改。Nginx 可负责 HTTPS 终止、证书自动续期、静态资源托管、访问日志和 \texttt{/api} 反向代理。课程验收环境以本地或测试服务器演示为主，重点验证密钥不写入前端代码和第三方异常可降级。
