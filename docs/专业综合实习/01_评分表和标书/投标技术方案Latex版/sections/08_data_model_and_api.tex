...
\begin{longtable}{|p{3.2cm}|p{4cm}|p{6.8cm}|}
  \caption{数据库核心表设计}
...
\begin{longtable}{|p{0.8cm}|p{4.2cm}|p{1.8cm}|p{4.6cm}|p{2.6cm}|}
  \caption{API 接口总览表}
  \label{tab:api-list}\\
  \toprule
  ...
  \hline
  7 & \path|/api/ai/fishing-advice| & POST & 根据推荐结果生成个性化出钓建议 & AI Agent \\
  \hline
  ...
  \hline
  15 & \path|/api/tutorials| & GET & 查询教程列表 & 教程 \\
  \hline
  16 & \path|/api/tutorials/{tutorial_id}/progress| & PUT & 更新学习进度 & 教程 \\
  \hline
  17 & \path|/api/users/{user_id}| & GET & 查询用户基础信息 & 用户 \\
  \hline
  18 & \path|/api/users/{user_id}/profile-summary| & GET & 获取个人统计摘要 & 个人报告 \\
  \hline
  19 & \path|/api/users/{user_id}/reports| & GET/POST & 查询或生成报告快照 & 个人报告 \\
  \hline
  20 & \path|/api/fish-species| & GET & 查询鱼种知识库 & 知识库 \\
\end{longtable}
